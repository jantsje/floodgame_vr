 ...
 */
 ...
 //HALLO ZIJN WE DAAR? of juist hier?
...
(function(angular){
	...
	app.constant('DEFAULT_NUM_COLS',20);
	app.constant('DEFAULT_NUM_BOMBS',40);
	app.constant('DEFAULT_BOXES_REQUIRED',30);
	app.constant('DEFAULT_BOX_VALUE',10000);
	...
	BombTaskController.prototype.update = function(column,active)
		{
			...
			if( active )
			{
                ...
                console.log('is bomb ', this.isBomb(column))
                ...
                console.log('total nr of bombs ', this.collectionBomb);
				column.$$active = true;
			}
			else
			{
	...
	BombTaskController.prototype.isBomb = function(column)
		{
		    ...
		    {
		        var eq = angular.equals(this.bomb[i], column);
		        if(eq)
		        {
		            return(true);
		        }
...
BombTaskController.prototype.getRemainingBoxes = function()
		{
			return Math.max(this.boxes_required - this.collectionBomb,0);
		};


BombTaskController.prototype.clickedAllBoxes = function()
		{
			if (this.getRemainingBoxes() <= 0)
			    {this.stopped = true;console.log(this.stopped, " is the status of stopped");}
            return this.getRemainingBoxes() <= 0;
		};
	...
	BombTaskController.prototype._initSettings = function()
		{
			...
			this.boxes_required = this._getConstant('BOXES_REQUIRED');
			this.box_value = this._getConstant('BOX_VALUE')
